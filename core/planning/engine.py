...
class PlanningEngine:
    ...
    def execute_next_step(self) -> Optional[PlanStep]:
        ...
        # For web search steps with time-sensitive queries, ensure correct year is used
        if step.tool_name == "web_search" and "query" in step.tool_args:
            query = step.tool_args["query"]
            current_year = self.execution_context.get('current_year')
            
            if current_year:
                # Check if query contains a year that might be incorrect
                years_to_check = ["2020", "2021", "2022", "2023", "2024", "2025", "2026"]
                contains_year = any(year in query for year in years_to_check)
                
                # Keywords indicating time-sensitive queries
                time_keywords = ["this year", "current year", "latest", "recent", "most recent", "last"]
                contains_time_keywords = any(keyword in query.lower() for keyword in time_keywords)
                
                if contains_year and contains_time_keywords:
                    original_query = query
                    # Find and replace incorrect year
                    for year in years_to_check:
                        if year in query and year != current_year:
                            step.tool_args["query"] = query.replace(year, current_year)
                            self.logger.info(f"QUERY CORRECTION: '{original_query}' → '{step.tool_args['query']}'")
                            break
                
                # If query has time keywords but no specific year, add current year
                elif contains_time_keywords and current_year not in query:
                    original_query = query
                    
                    # Handle "this year" and similar phrases
                    if "this year" in query.lower():
                        step.tool_args["query"] = query.replace("this year", current_year)
                    elif "current year" in query.lower():
                        step.tool_args["query"] = query.replace("current year", current_year)
                    # Super Bowl specific handling
                    elif "super bowl" in query.lower() or "superbowl" in query.lower():
                        step.tool_args["query"] = f"{query} {current_year}"
                    else:
                        # For other time-sensitive queries
                        step.tool_args["query"] = f"{query} {current_year}"
                    
                    self.logger.info(f"ADDED YEAR TO QUERY: '{original_query}' → '{step.tool_args['query']}'")
