...
class LLMManager:
    ...
    def generate_plan(self, goal: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Extract temporal context from context (should be available directly now)
        current_date = context.get('current_date')
        current_year = context.get('current_year')
        
        # Log availability of temporal context
        if current_date or current_year:
            self.logger.info(f"Temporal context available: date={current_date}, year={current_year}")
        else:
            self.logger.warning("No temporal context found in planning context")
            # Try to extract from config metadata as fallback
            if 'config' in context and 'metadata' in context['config']:
                ...
                current_year = config_metadata.get('current_year')
                if current_date or current_year:
                    self.logger.info(f"Extracted temporal context from config: date={current_date}, year={current_year}")
        ...
        system_message = f"""You are an AI assistant that can break down tasks into specific steps.
...
"""
    ...
    def generate_response(self, message: str, context: Dict[str, Any]) -> str:
        ...
        # Extract temporal context directly from context first
        current_date = context.get('current_date')
        current_year = context.get('current_year')
        
        # If not found directly, try from config metadata
        if not current_date and not current_year:
            config_metadata = context.get('config', {}).get('metadata', {})
            current_date = config_metadata.get('current_date')
            current_year = config_metadata.get('current_year')
            
            # Log what we found
            if current_date or current_year:
                self.logger.info(f"Using temporal context from config metadata: date={current_date}, year={current_year}")
            else:
                self.logger.warning("No temporal context found for response generation")
        else:
            self.logger.info(f"Using direct temporal context: date={current_date}, year={current_year}")
...
Be helpful, informative, and concise."""
        ...
        if current_date or current_year:
            temporal_context = f"\nIMPORTANT TEMPORAL CONTEXT: Today's date is {current_date or 'unknown'}. When processing queries about any other time-relative terms use this information as your reference point."
            system_message += temporal_context
            self.logger.info("Added temporal context to response system message")
        
        # Log the full system message for debugging
        self.logger.info(f"System message for response: {system_message}")
...
"""
    ...
    def reevaluate_plan(self, goal: str, current_plan: Dict[str, Any], 
        ...
        # Extract temporal context from config metadata (if available)
        config_metadata = context.get('config', {}).get('metadata', {})
        current_date = config_metadata.get('current_date', None)
        current_year = config_metadata.get('current_year', None)
        ...
        # Construct prompt for the LLM
        system_message = """You are an AI assistant that can analyze execution results and adapt plans accordingly.
