...
class WebSearchTool(Tool):
    ...
    def __init__(self, 
        ...
        Args:
            ...
            api_key: API key for the search engine (if None, will try to load from environment variables)
            search_engine: Search engine to use (google, bing, duckduckgo, serper)
            cx_id: Custom Search Engine ID (required for Google, if None will try to load from environment variables)
        ...
        # Set API key based on search engine, using env vars as fallback
        if api_key is None:
        ...
        # Set CX ID for Google, using env var as fallback
        if search_engine == "google" and cx_id is None:
        ...
        if search_engine in ["bing", "serper"] and not api_key:
            raise ValueError(f"API key is required for {search_engine} search. "
                           f"Set it directly or via {'BING_API_KEY' if search_engine == 'bing' else 'SERPER_API_KEY'} environment variable.")
